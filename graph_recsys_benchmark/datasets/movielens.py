...
def reindex_df_mlsmall(movies, ratings, tagging):
    ...
    """
    # Reindex uid
    unique_uids = np.sort(ratings.uid.unique()).astype(np.int64)
    uids = np.arange(unique_uids.shape[0]).astype(np.int64)
    raw_uid2uid = {raw_uid: uid for raw_uid, uid in zip(unique_uids, uids)}
    ratings['uid'] = np.array([raw_uid2uid[raw_uid] for raw_uid in ratings.uid], dtype=np.int64)
    tagging['uid'] = np.array([raw_uid2uid[raw_uid] for raw_uid in tagging.uid], dtype=np.int64)

    # Reindex iid
    unique_iids = np.sort(movies.iid.unique()).astype(np.int64)
    iids = np.arange(unique_iids.shape[0]).astype(np.int64)
    raw_iid2iid = {raw_iid: iid for raw_iid, iid in zip(unique_iids, iids)}
    movies['iid'] = np.array([raw_iid2iid[raw_iid] for raw_iid in movies.iid], dtype=np.int64)
    ratings['iid'] = np.array([raw_iid2iid[raw_iid] for raw_iid in ratings.iid], dtype=np.int64)
    tagging['iid'] = np.array([raw_iid2iid[raw_iid] for raw_iid in tagging.iid], dtype=np.int64)

    # Create tid
    unique_tags = np.sort(tagging.tag.unique()).astype(np.str)
    tids = np.arange(unique_tags.shape[0]).astype(np.int64)
    tags = pd.DataFrame({'tid': tids, 'tag': unique_tags})
    tag2tid = {tag: tid for tag, tid in zip(unique_tags, tids)}
    tagging['tid'] = np.array([tag2tid[tag] for tag in tagging.tag], dtype=np.int64)
...
def reindex_df_ml25m(movies, ratings, tagging, genome_tagging, genome_tags):
    ...
    ratings['iid'] = np.array([raw_iid2iid[raw_iid] for raw_iid in ratings.iid], dtype=np.int64)
    tagging['iid'] = np.array([raw_iid2iid[raw_iid] for raw_iid in tagging.iid], dtype=np.int64)
    genome_tagging['iid'] = np.array([raw_iid2iid[raw_iid] for raw_iid in genome_tagging.iid], dtype=np.int64)
    ...
    # Reindex genome_tid
    unique_genome_tids = np.sort(genome_tags.genome_tid.unique()).astype(np.int64)
    genome_tids = np.arange(unique_genome_tids.shape[0]).astype(np.int64)
    ...
    genome_tags['genome_tid'] = np.array(
        [raw_genome_tid2genome_tid[raw_genome_tid] for raw_genome_tid in genome_tags.genome_tid], dtype=np.int64)
...
class MovieLens(Dataset):
    ...
    def __init__(self,
        ...
        assert self.name in ['25m', '20m', 'latest-small', '100k']
    ...
    def process(self):
        if self.name == '25m':
            try:
                ...
                movies['year'] = years
                if self.type == 'hete':
                    years = movies.year.to_numpy().astype(np.int64)
                    min_year = min(years)
                    max_year = max(years)
            ...
            try:
                ...
                ratings = ratings[ratings.timestamp > 1388534399]     #2M interactions
