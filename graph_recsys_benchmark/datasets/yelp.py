...
class Yelp(Dataset):
    ...
    def process(self):
        ...
        if isfile(join(self.processed_dir, 'business.pkl')) and isfile(join(self.processed_dir, 'user.pkl')) and isfile(
            ...
            business = pd.read_parquet('/Users/chuanchon/Desktop/PEAGNN/Yelp-1000user/business.pq')
            user = pd.read_parquet('/Users/chuanchon/Desktop/PEAGNN/Yelp-1000user/user.pq')
            review = pd.read_parquet('/Users/chuanchon/Desktop/PEAGNN/Yelp-1000user/review.pq')
            tip = pd.read_parquet('/Users/chuanchon/Desktop/PEAGNN/Yelp-1000user/tip.pq')
            checkin = pd.read_parquet('/Users/chuanchon/Desktop/PEAGNN/Yelp-1000user/checkin.pq')

            print('Preprocessing...')

            # SONCX: to slice data of 1000 users that rating the most
            grouped_rating = review.groupby('user_id').apply(lambda x: x['business_id'].to_list()).reset_index(name='business_id')
            grouped_rating['lens_inter'] = grouped_rating["business_id"].apply(lambda x: len(x))
            grouped_rating = grouped_rating.sort_values(by='lens_inter', ascending=False)
            li_user = grouped_rating.iloc[:1000]['user_id'].tolist()
            li_business = grouped_rating['business_id'].explode().tolist()

            business = business[business['business_id'].isin(li_business)]
            review = review[review['user_id'].isin(li_user)]
            user = user[user['user_id'].isin(li_user)]
            checkin = checkin[checkin['business_id'].isin(li_business)]
            tip = tip[tip['user_id'].isin(li_user)]

            print(f'Business shape: {business.shape}')
            print(f'Business - business: {len(business["business_id"].unique().tolist())}')
            print()
            print(f'Review shape: {review.shape}')
            print(f'Review - business: {len(review["business_id"].unique().tolist())}')
            print(f'Review - user: {len(review["user_id"].unique().tolist())}')
            print()
            print(f'User shape: {user.shape}')
            print(f'User - user: {len(user["user_id"].unique().tolist())}')
            print()
            print(f'Checkin shape: {checkin.shape}')
            print(f'Checkin - business: {len(checkin["business_id"].unique().tolist())}')
            print()
            print(f'Tip shape: {tip.shape}')
            print(f'Tip - user: {len(tip["user_id"].unique().tolist())}')
            print(f'Tip - business: {len(tip["business_id"].unique().tolist())}')
